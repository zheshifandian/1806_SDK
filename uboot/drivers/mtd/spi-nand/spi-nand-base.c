...
void spi_read(struct spi_nand_chip *chip, loff_t from, size_t len, u_char *buf)
{
	size_t retlen;
	spi_nand_read(chip, from, len, &retlen, buf);
}

void spi_write(struct spi_nand_chip *chip, loff_t to, size_t len, const u_char *buf)
{
	size_t retlen;
	spi_nand_write(chip, to, len, &retlen, buf);
}

void spi_erase(struct spi_nand_chip *chip, loff_t off,size_t len)
{
	spi_nand_erase(chip, off, len, false);
}
