                                               ...
                                               bool force_client_disconnect)
{
    ...
    if (n_type == beerocks::TYPE_CLIENT) {
        ...
        node->rx_rate = database.get_load_rx_phy_rate_100kb(n->mac);
        node->tx_rate = database.get_load_tx_phy_rate_100kb(n->mac);
    } else {
        if (n->parent_mac != std::string()) {
            node->rx_rssi = database.get_node_link_rssi(n->parent_mac);
