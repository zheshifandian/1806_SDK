...
int main(int argc, char* argv[]){
	...
	if (ifindex) {
		//printf("[adapt info]start to check ifindex : %d\n",ifindex);
	} else {
		printf("[adapt info]can't find ifindex of %s\n",iface);
		return -1;
	}
	...
	if (result < 0) {
		printf("[adapt error]send packet to %s failed, reason code:%d\n", iface, result);
		return result;
	}
	...
	if (*message == DHCPOFFER) {
		FILE *fp = fopen("/tmp/dhcp_iface", "w+");
		fprintf(fp, "%s\n", iface);
		fclose(fp);
		printf("[adapt info]receive dhcp offer in ifindex %d, start setting\n",ifindex);
	} else {
		printf("[adapt info]no dhcp offer received from %s\n", iface);
	}
	return 0;
}
