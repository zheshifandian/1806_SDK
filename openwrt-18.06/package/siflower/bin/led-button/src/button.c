...
void evdev_handler(int signum)
{
	...
	int count, sec[2], usec[2];
	struct board_name *bd;
	int ret, press_time, i = 0;

	debug(LOG_DEBUG, "%s: signum is %d vs SIGIO is %d\n",
						__func__, signum, SIGIO);
	if (fd < 0) {
		debug(LOG_DEBUG, "Input device not found! fd = %d\n", fd);
		return;
	}
	...
	bd = plat_get_board();
	if (!bd) {
		debug(LOG_DEBUG, "In %s : plat_get_board fail !!!\n", __func__);
		return;
	}
	debug(LOG_DEBUG, "%d %d %d %d\n", count, evt.type, evt.value, evt.code);
	while (count > 0) {
		/*
		...
		if (evt.type == EV_KEY) {
			...
			if (last[i].code == KEY_MAX) {
				...
				ret = plat_btn_handler(BTN_ACTION_EDGE, 0, bd, evt.code);
				if (ret == 0) {
					event_handled(i);
					goto set_done;
				}
				if (evt.value)
					ret = plat_btn_handler(BTN_ACTION_HIGH, 0, bd, evt.code);
				else
					ret = plat_btn_handler(BTN_ACTION_LOW, 0, bd, evt.code);
				if (ret == 0)
					event_handled(i);
				else
			...
			press_time = sec[0] - sec[1];
			ret = plat_btn_handler(BTN_ACTION_PRESS, press_time, bd, evt.code);
			event_handled(i);
			if (ret != 0)
