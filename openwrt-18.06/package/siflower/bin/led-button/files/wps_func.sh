...
set_channel() {
	local num=0
	local theinfo=`iw "$1" info`
	local chan=`iwinfo "$1" info | grep Chan|awk -F ' ' '{print $4}'`
	local freq=`echo $theinfo | awk -F ')' '{print $1}' | awk -F '(' '{print $2}' | awk -F ' ' '{print $1}'`
	local width=`echo $theinfo | awk -F ',' '{print $2}' | awk -F ' ' '{print $2}'`
	local center1=`echo $theinfo | awk -F ',' '{print $3}' | awk -F ' ' '{print $2}'`
	local htmode40="HT40"
	case "$1" in
		sfi0)
			num=0
	...
	[ "$chan" -gt 0 ] && {
		#keep htmode of radio0 same as htmode of sfi0(2.4g).
		[ "$num" = 0 ] && {
			[ "$width" = 40 ] && {
				[ "$center1" -lt "$freq" ] && htmode40="HT40-"
				[ "$center1" -gt "$freq" ] && htmode40="HT40+"
				uci set wireless.radio${num}.htmode="$htmode40"
				uci set wireless.radio${num}.noscan="1"
			}
			#fix the problem that the htmode of repeater won't be changed when the superior route change into 20MHz.
			[ "$width" = 20 ] && {
				uci set wireless.radio${num}.htmode="HT20"
			}
		}
