...
int postDataToHttpd(char *command,char *postData,void *response,char *userId, int32_t count)
{
    ...
    if(!sp) LOG("new device listen get sp sfroot shadow fail\n");
    CURLcode ret;
    ...
    curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, TIMEOUT_UHTTPD_S);
    curl_easy_setopt(curl, CURLOPT_TIMEOUT, TIMEOUT_UHTTPD_S);
    ...
    uint32_t timeDelta = time(NULL) - time1;
    long responseCode;
    curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &responseCode);
    //LOG("perform2 curl_easy_perform------2----responsecode %ld cost time %d s\n",responseCode,timeDelta);
	...
	if((responseCode == UHTTPD_REDIRECT_URL_CODE) && count){
		...
		return postDataToHttpd(command,postData,response,userId,count - 1);
	}
...
{
    return postDataToHttpd(command,postData,response,USER_COMMON,1);
}
