...
//				uciSetValue(ctx, &ptr, "ip", device_ip);
        ...
        uciSetValue(ctx, &ptr, "latest_time", time_now);
        uci_save(ctx, pkg);
    ...
    //Now arp table can't get device ip, so use script to update ip later
    sprintf(cmd, "/usr/sbin/updatewldevlist.sh %s &", device_macaddr);
    system(cmd);
    return ret;
}
