...
int parse_data(cJSON* root){
	...
	sscanf(p_jmac->valuestring, "%02x_%02x_%02x_%02x_%02x_%02x", &mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]);
...
int32_t traverse_urllist(unsigned char * map_addr, unsigned char operation){
	...
	char url[51] = {'\0'};
	char listtype = 0;
	char * flat_url_list = map_addr + 9 + 9*node_count;
	char *flat_mac_node_hdr = NULL;
