...
static int sf_factory_read2(char *data, int len, char *ps)
{
	FILE *ptr = NULL;
    char buff[128];
	int i = 0;
...
int sf_factory_read_operation(enum sf_factory_read_action action, char* data, int len)
{
	...
	switch (action) {
		...
		case READ_FIRST_MAC_ADDRESS:
			...
			break;
		case READ_VENDER:
			strcpy(ps,"cat /sys/devices/platform/factory-read/vender");
			goto read_char;
		case READ_VENDER_FLAG:
			strcpy(ps,"cat /sys/devices/platform/factory-read/vender_flag");
			goto read_char;
		case READ_PRODUCT_KEY:
			strcpy(ps,"cat /sys/devices/platform/factory-read/product_key");
			goto read_char;
		case READ_PRODUCT_KEY_FLAG:
			strcpy(ps,"cat /sys/devices/platform/factory-read/product_key_flag");
			goto read_char;
		default:break;
	}
	...
	return 0;
read_char:
    if(sf_factory_read2(data, len, ps))
        return -1;
	return 0;
}
