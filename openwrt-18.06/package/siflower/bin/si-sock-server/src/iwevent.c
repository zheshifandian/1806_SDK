...
void *checknewdev_process(custom_iface_ctx *custom_and_iface)
{
    ...
    if(strcmp("00:00:00:00:00:00", (const char *)mac) == 0)
    {
    ...
    if(uci_load(ctx, WLDEVLIST, &pkg) == UCI_OK)
    {
        ...
        strcpy(mac_section_name, (const char *)mac);
        ...
        {
			if(strstr(interface,"rai") != NULL || strstr(interface,"sfi") != NULL){
    ...
    strcpy(device->macaddr, (const char *)mac);
...
// TODO add wireless attr here
	...
	if(device->newdevice){
		if(strstr(device->interface,"lease") == NULL){
			...
			sprintf(cmd,"aclscript l_add %s; tscript add %s", mac,mac);
			DB_LOG("%s",cmd);
			ret = system(cmd);
			if(ret == -1)
...
void *check_new_wire_dev(void *arg){
	...
	ret = postDataToHttpdCommon(LOCAL_COMMAND_DEVICE_LIST_BACKSTAGE,postData,&response);
...
void handle_dps_netlink_event(struct rtnl_handle *rth){
	struct sockaddr_nl sanl;
	msgtemplate_t *genlmsg;
	socklen_t sanllen = sizeof(struct sockaddr_nl);
	int amt;
...
void handle_custom_wifi_events(struct rtnl_handle *rth)
{
	struct sockaddr_nl sanl;
	socklen_t sanllen = sizeof(struct sockaddr_nl);
	int buf_len = 0;
...
void handle_usb_netlink_uevent(struct rtnl_handle *rth){
	char buf[256] = {0};
	char data[32];
	int ret, usb_action ;
...
void *arpevent_process(void *arg){
	...
	sprintf(postData,"{\"version\":\"V10\",\"ip\":\"%s\",\"mac\":\"%s\"}",ipaddr,macaddr);
...
void do_arp_event(struct ndmsg *ndm, u_int32_t ndml){
	...
	if(ndm->ndm_family == AF_INET && (ndm->ndm_state & NUD_REACHABLE) && ndm->ndm_ifindex != wan_ifi){
		...
		sprintf(tmp_mac,"%02hhX_%02hhX_%02hhX_%02hhX_%02hhX_%02hhX",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
...
static inline void handle_arp_netlink_event(struct rtnl_handle *  rth)
{
	struct sockaddr_nl sanl;
	socklen_t sanllen = sizeof(struct sockaddr_nl);
