...
/*	Change log level in etc/config/sidefault.ssstlog.level
 *	0 - Close any LOG
 *	1 - Only LOG, no DB_LOG
 *	2 - Open LOG and DB_LOG
*/
#define DB_LOG(X,...) {	\
	if(get_log_level()>1) {	\
		syslog(LOG_CRIT,X,##__VA_ARGS__);	\
	}	\
}
...
//product key related info
#define PRODUCT_LEN 33
#define PRODUCT_OFFSET 121
#define PRODUCT_VERIFY_OFFSET 119
#define PRODUCT_VERIFY_VALUE "pk"
#define ROUTER_KEY_PATH "/etc/routerkey"
