...
int coap_parse_header(coap_header_t *hdr, const uint8_t *buf, size_t buflen)
{
    if (buflen < 4)
...
int coap_parse_token(coap_buffer_t *tokbuf, const coap_header_t *hdr, const uint8_t *buf, size_t buflen)
{
	if (hdr->tkl == 0) {
		tokbuf->p = NULL;
		tokbuf->len = 0;
...
// advances p
int coap_parse_option(coap_option_t *option, uint16_t *running_delta, const uint8_t **buf, size_t buflen)
{
    const uint8_t *p = *buf;
    uint8_t headlen = 1;
    uint16_t len, delta;
...
int coap_parse_options_and_payload(coap_option_t *options, uint8_t *numOptions,
		coap_buffer_t *payload, const coap_header_t *hdr, const uint8_t *buf, size_t buflen)
{
    ...
    {
        if (0 != (rc = coap_parse_option(&options[optionIndex], &delta, &p, end-p)))
            return rc;
        optionIndex++;
    }
...
int coap_parse(coap_packet_t *pkt, const uint8_t *buf, size_t buflen)
{
    ...
    if (0 != (rc = coap_parse_header(&pkt->hdr, buf, buflen)))
        return rc;
    if (0 != (rc = coap_parse_token(&pkt->tok, &pkt->hdr, buf, buflen)))
        return rc;
    pkt->numopts = MAXOPT;
    if (0 != (rc = coap_parse_options_and_payload(pkt->opts, &(pkt->numopts), &(pkt->payload), &pkt->hdr, buf, buflen)))
        return rc;
    return 0;
}
