...
{
    UI32_T check_bit, i, reg, value, offset;
...
{
    UI32_T data_en[4], data_end[4], data_rev[4];
...
air_acl_setRule(
    const UI32_T            unit,
    const UI32_T            rule_idx,
    const AIR_ACL_RULE_T    rule)
{
    UI32_T  type0_key[12], type0_mask[12];
    UI32_T  type1_key[12], type1_mask[12];
    UI32_T  type0_t[12], type0_c[12];
    UI32_T  type1_t[12], type1_c[12];
    AIR_ACL_CTRL_T ctrl;

    /* Check parameter */
    AIR_PARAM_CHK((rule_idx >= ACL_MAX_RULE_NUM), AIR_E_BAD_PARAMETER);
    AIR_PARAM_CHK((rule.key.stag > BITS_RANGE(0, STAG_WIDTH)), AIR_E_BAD_PARAMETER);
    AIR_PARAM_CHK((rule.key.ctag > BITS_RANGE(0, CTAG_WIDTH)), AIR_E_BAD_PARAMETER);
    AIR_PARAM_CHK((rule.key.etype > BITS_RANGE(0, ETYPE_WIDTH)), AIR_E_BAD_PARAMETER);
    AIR_PARAM_CHK((rule.key.dscp > BITS_RANGE(0, DSCP_WIDTH)), AIR_E_BAD_PARAMETER);
    AIR_PARAM_CHK((rule.key.protocol > BITS_RANGE(0, PROTOCOL_WIDTH)), AIR_E_BAD_PARAMETER);
    AIR_PARAM_CHK((rule.key.flow_label > BITS_RANGE(0, FLOW_LABEL_WIDTH)), AIR_E_BAD_PARAMETER);
    AIR_PARAM_CHK((rule.key.dport > BITS_RANGE(0, DPORT_WIDTH)), AIR_E_BAD_PARAMETER);
    AIR_PARAM_CHK((rule.key.sport > BITS_RANGE(0, SPORT_WIDTH)), AIR_E_BAD_PARAMETER);
    AIR_PARAM_CHK((rule.key.udf > BITS_RANGE(0, UDF_WIDTH)), AIR_E_BAD_PARAMETER);
...
{
    I32_T   ret;
    UI32_T  value, trtcmr1, trtcmr2, trtcmr3, trtcmr4;
    ...
    const UI32_T low)
{
    ...
    u32dat = (u32dat & ~(DPCR_LOW_THRSH_WIDTH)) | low;
    u32dat = (u32dat & ~(DPCR_HIGH_THRSH_WIDTH << DPCR_HIGH_THRSH_OFFSET)) | (high << DPCR_HIGH_THRSH_OFFSET);
    ...
    UI32_T *ptr_low)
{
    ...
    aml_readReg(unit, DPCR(port, color, queue), &u32dat);
    *ptr_low = u32dat & DPCR_LOW_THRSH_WIDTH;
    *ptr_high = (u32dat >> DPCR_HIGH_THRSH_OFFSET) & DPCR_HIGH_THRSH_WIDTH;
    return AIR_E_OK;
    ...
    const UI32_T probability)
{
    ...
    aml_readReg(unit, DPCR(port, color, queue), &u32dat);
    u32dat = (u32dat & ~(DPCR_PBB_WIDTH << DPCR_PBB_OFFSET)) | (probability << DPCR_PBB_OFFSET);
    aml_writeReg(unit, DPCR(port, color, queue), u32dat);
    return AIR_E_OK;
