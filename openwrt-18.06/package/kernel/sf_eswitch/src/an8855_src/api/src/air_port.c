...
{
    UI32_T u32dat;
    UI32_T u32cl45_1e_3c;
    UI32_T u32cl45_1e_3d;
    UI32_T u32cl45_1e_3e;
    ...
    {
        /* Enable EEE */
        u32dat =  ( RG_SYSPLL_DDSFBK_EN | RG_SYSPLL_DMY1 );
        u32dat |= ( RG_SYSPLL_EEE_EN | RG_SYSPLL_EEE_EN_PYPASS ) ;
        u32dat |= ( RG_SYSPLL_EFUSE_DIS | RG_XSQ_LPF_EN ) ;
        aml_writePhyRegCL45(unit, port, PHY_DEV_1FH, PLL_GROUP_CONTROL_REG, u32dat);
        ...
        u32dat = 1;
        aml_writePhyRegCL45(unit, port, PHY_DEV_1EH, DSP_FRE_PM_REG, u32dat);

        u32dat = (DSP_FRE_RP_FSM_EN | DSP_FRE_DW_AUTO_INC | DSP_FRE_WR_EN | DSP_FRE_SW_RST);
        aml_writePhyRegCL45(unit, port, PHY_DEV_1EH, DSP_FRE_REG, u32dat);

        u32dat = (RG_LPI_VCO_EEE_STGO_EN | RG_LPI_TR_READY | RG_LPI_SKIP_SD_SLV_TR | VCO_SLICER_THRES_H);
        aml_writePhyRegCL45(unit, port, PHY_DEV_1EH, RG_LPI_REG, u32dat);

        /* Set TX_PAIR_DELAY_SEL gbe mode */
        aml_writePhyRegCL45(unit, port, PHY_DEV_1EH, TX_PAIR_DELAY_SEL_REG, 0x0);
        aml_writePhyRegCL45(unit, port, PHY_DEV_1EH, DSP_CONTROL_REG, PICMD_MISER_MODE_INT(0x771));


        /* Set page to external reg */
        aml_writePhyReg(unit, port, PHY_DEV_1FH, 0x52b5);
        /* Set LSB value */
        aml_writePhyReg(unit, port, DFETAILDC_COEFF_L, 0x002b);
        /* Set MSB value */
        aml_writePhyReg(unit, port, DFETAILDC_COEFF_M, 0x0000);
        /* command: trigger & write */
        aml_writePhyReg(unit, port, EXPHY_CMD_WRITE, 0x8F80);
        /* Restore page */
        aml_writePhyReg(unit, port, PHY_DEV_1FH, 0x0000);
    }
    else
    {
        /* Disable EEE */
