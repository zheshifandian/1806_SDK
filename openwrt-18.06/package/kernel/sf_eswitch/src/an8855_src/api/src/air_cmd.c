...
static UI32_T _strtoul(const C8_T *cp, C8_T **endp, UI32_T base);
        ...
        C8_T *argv[])
{
    ...
    if (7 == argc)
    {
        sscanf(argv[0],"mode=%s",str);
        if(_strcmp(str,"replace") == 0)
            mode = AIR_STAG_MODE_REPLACE;
        else if(_strcmp(str,"insert") == 0)
            mode = AIR_STAG_MODE_INSERT;
        else
            printf("mode is wrong!!");


        sscanf(argv[1],"opc=%s",str);
        if(_strcmp(str,"portmap") == 0)
            sptag_tx.opc = AIR_STAG_OPC_PORTMAP;
        else if(_strcmp(str,"portid") == 0)
            sptag_tx.opc = AIR_STAG_OPC_PORTID;
        else if(_strcmp(str,"lookup") == 0)
            sptag_tx.opc = AIR_STAG_OPC_LOOKUP;
        else
            printf("opc is wrong!!");

        sscanf(argv[2],"dp=%x",&data);
        sptag_tx.pbm = data;
        AIR_PRINT("sptag_tx.pbm %x\n",sptag_tx.pbm);

        sscanf(argv[3],"vpm=%s",str);
        if(_strcmp(str,"untagged") == 0)
            sptag_tx.vpm = AIR_STAG_VPM_UNTAG;
        else if(_strcmp(str,"8100") == 0)
            sptag_tx.vpm = AIR_STAG_VPM_TPID_8100;
        else if(_strcmp(str,"88a8") == 0)
            sptag_tx.vpm = AIR_STAG_VPM_TPID_88A8;
        else
            printf("vpm is wrong!!");

        sscanf(argv[4],"pri=%d",&data);
        sptag_tx.pri = data;
        AIR_PRINT("sptag_tx.pri %d\n",sptag_tx.pri);

        sscanf(argv[5],"cfi=%d",&data);
        sptag_tx.cfi  = data;
        AIR_PRINT("sptag_tx.cfi %d\n",sptag_tx.cfi);

        sscanf(argv[6],"vid=%d",&data);
        sptag_tx.vid = data;
        AIR_PRINT("sptag_tx.vid %d\n",sptag_tx.vid);

        ret = air_sptag_encodeTx(0,mode, &sptag_tx, &buf, &len);
        if(AIR_E_OK == ret)
        {
...
doSaLearning(UI32_T argc, C8_T *argv[])
{
    ...
    {
        rc = air_sec_getPortSecPortCfg(0, port, &port_config.sa_lrn_en);
        if(AIR_E_OK == rc)
        {
        ...
        C8_T *argv[])
{
    AIR_ERROR_NO_T ret = AIR_E_OK;
    UI32_T rule_idx;
    BOOL_T state, reverse, end;
    UI32_T argi = 0, ipv6, i;
    AIR_ACL_RULE_T rule;
    UI8_T tmp_ip[16] = {0};
    C8_T str_temp[AIR_MAX_NUM_OF_PORTS+1];
    C8_T str[40];
