...
mac802111_get_apmacidx(){
	...
	#what if $mask == "ff:ff:ff:ff:ff:ff"
	[ "$((0x$mask1))" -gt 0  ] && {
		macidx=$((0x$mask1))
		return
	}
	...
	[ "$((0x$mask6))" -lt 255  ] && macidx=$((0x$mask6))
}
...
mac80211_prepare_vif() {
	...
	#TODO rm .vlan what does it use for?
	...
	case "$mode" in
		...
		sta|wds-sta)
			...
			#TODO do we really need brctl here?
