...
{
#ifdef NEW_SCHEDULE
    /// WLAN driver call-back function: send_data_cfm_burst
...
struct ipc_host_env_tag
{
    ...
    uint32_t rx_bufsz;
    // Store the offset of rx payload
    uint32_t rx_pd_offset;
    // max rx process num per irq
...
/**
 ******************************************************************************
 * @brief DeInitialize the IPC running on the Application CPU.
 *
 * This function:
 *  --free all host id resource in ipc_host_init, this is just for new_schedule methodoly
 *
 * @param[in]   env   Pointer to the IPC host environment
 *
 ******************************************************************************
...
/**
 ******************************************************************************
 * @brief Clear IPC status on the host side.
 *
 * @param[in]   status   IPC host status
 *
 ******************************************************************************
 */
...
/**
 ******************************************************************************
 * @brief process hostids already pushed in ipc_hostid[]
 *
 * @param[in]   env  Global ipc_host environment pointer
 * @param[in]   queue_idx  hwq index
 * @param[in]   user_pos   userpos of ipc_hostid[]
 * @param[in]   process_host_id call back function to process host ids
 *
 * @warning After calling this function, IPC interrupts can be triggered at any
 * time. Potentially, an interrupt could happen even before returning from the
 * function if there is a request pending from the embedded side.
