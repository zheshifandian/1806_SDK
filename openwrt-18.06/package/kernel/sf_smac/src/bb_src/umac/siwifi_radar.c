...
/**
 * dfs_pattern_detector_exit() - delete all channel detectors
 *
 * @dpd: dfs_pattern_detector
 */
...
void siwifi_radar_detected(struct siwifi_hw *siwifi_hw)
{
    ...
    /*
      recopy chan_def in local variable because siwifi_radar_cancel_cac may
...
/*
 * when radar detected, printk last 32 pulses
 */
...
static void siwifi_radar_process_pulse(struct work_struct *ws)
{
    ...
    for (chain = SIWIFI_RADAR_RIU; chain < SIWIFI_RADAR_LAST; chain++) {
        ...
        for (i = 0; i < pulses_count[chain] ; i++) {
            ...
            // RM#14310 Some values may be wrong when restart driver (sfwifi reset fmac)
            if (!p){
                continue;
            }
