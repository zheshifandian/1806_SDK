...
sub download_cmd($) {
	...
	return $have_curl
		? (qw(curl -f --connect-timeout 8 --retry 2 --location --insecure), shellwords($ENV{CURL_OPTIONS} || ''), $url)
		: (qw(wget --tries=2 --timeout=8 --no-check-certificate --output-document=-), shellwords($ENV{WGET_OPTIONS} || ''), $url)
	;
}
