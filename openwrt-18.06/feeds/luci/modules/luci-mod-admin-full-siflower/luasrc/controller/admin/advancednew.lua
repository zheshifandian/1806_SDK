...
function index()
	...
	if (wds_enable == false) then
		...
		--wan/lan自适应
		local if_support = uci:get("basic_setting","auto_adapt","support") or 0
		if(tonumber(if_support) == 1) then
			file = io.popen("ls /bin | grep auto_adapt")
			local adapt_en = file:read("*all")
			if string.match(adapt_en,"auto_adapt") then
				entry({"admin", "advancednew", "adapt"}, template("new_siwifi/senior_user/adapt") , _("AUTO WAN-LAN"), 6);
				entry({"admin", "advancednew", "set_port"}, call("set_port")).leaf = true;
			end
		end
	else
...
function get_dmz_host()
	...
	local result = {
		...
		ipaddr = ipaddr or _uci_real:get("basic_setting","dmz","host") or "" --DMZ主机的ip地址
	}
	sysutil.nx_syslog(myprint(result), 1)
	luci.http.prepare_content("application/json")
	luci.http.write_json(result)
...
function get_ddns()
	...
	local result = {
		...
		login = _uci_real:get(cfg, sec, "enabled")
	}
	sysutil.nx_syslog(myprint(result), 1)
	luci.http.prepare_content("application/json")
	luci.http.write_json(result)
...
function get_devices()
    ...
    if old_time == nil or old_time == "" then
        old_time = 0
    else
	old_time = tonumber(old_time) or 0
    end
	...
	for i=1, #(dev_list) do
		...
		if is_ap == 0 then
			...
			for k, v in pairs(s) do
				if v.macaddr == string.lower(tostring(dev_list[i]["mac"])):gsub("_", ":") then
					devices[#devices]["hostname"] = v.hostname --设备名
				end
			end

			if manager_mac == dev_list[i]["mac"]:gsub("_", ":") then
...
function set_device()
	...
	if mac then
		...
		if internet then
			if internet == 0 then
				luci.util.exec("echo 2 %s > /proc/ts"%{mac_fmt}) -- add dev to hnat blacklist
			else
				luci.util.exec("echo 3 %s > /proc/ts"%{mac_fmt}) -- del dev from hnat blacklist
			end
		...
		if speed_ul and speed_dl then
			if speed_ul == -1 and speed_dl == -1 then
				...
				luci.util.exec("echo 2 %s > /proc/ts"%{mac_fmt}) -- add dev to hnat blacklist
				luci.util.exec("echo 4 %s > /proc/ts"%{mac_fmt}) -- reset tx/rx count
			...
			_uci_real:set(op_list, dev_mac, "limitup", tostring(speed_ul))
			_uci_real:set(op_list, dev_mac, "limitdown", tostring(speed_dl))
		end
		_uci_real:save(op_list)
		_uci_real:commit(op_list)
...
function set_port()
	...
	elseif cmd == "get" then
		...
		result["wan"] = file:read("*a"):match("%d")
		file:close()
		...
		result["enable"] = uci:get("network","lan","auto_adapt") or 0
	else
		result["set"] = "set"..cmd
