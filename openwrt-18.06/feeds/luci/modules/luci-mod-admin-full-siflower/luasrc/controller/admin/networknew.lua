...
function index()
    ...
    entry({"admin", "networknew", "get_ipv6"}, call("get_ipv6")).leaf = true;
    entry({"admin", "networknew", "set_ipv6"}, call("set_ipv6")).leaf = true;
...
function mac_to_ip(mac)
	local ipaddr = nil
	ipc.neighbors({ family = 4 }, function(n)
		if tostring(n.mac) == mac and n.dest then
...
function calc_lan_dhcp(check_ip_start, check_ip_end )
	...
	if (check_ip_val_start < top_ip_val and check_ip_val_start > bottom_ip_val and check_ip_val_end < top_ip_val and check_ip_val_end > bottom_ip_val) then
		...
		range = check_ip_val_end - check_ip_val_start + 1	--If not +1, the end ip address in Address Pool can't use
...
function get_wan()
	...
	mtu = _uci_real:get("network","wan","mtu")
	if mtu then
		mtu = mtu - 4
	end
	...
	if (proto == "dhcp") then
		result = {
			...
			DHCP = {
				...
				packageMTU = mtu or 1500, --int 数据包MTU(字节)
	...
	elseif (proto == "pppoe") then
		...
		result = {
			...
			PPPoE = {
				...
				packageMTU = mtu or 1480, --int 数据包MTU(字节)
...
function set_wan_dhcp(arg_list_table,result, releaseip)
	...
	local wan_ifc = _uci_real:get("network","wan","ifname")
	local old_mtu = tonumber(_uci_real:get("network","wan","mtu"))
	local mtu = tonumber(arg_list_table["packageMTU"])
	if mtu then mtu = mtu + 4 end -- reserve switch vlan tag room here
	if mtu and mtu ~= old_mtu then
		if mtu < 42 or mtu > 9004 then
		...
		_uci_real:set("network","eth0","mtu",mtu)
	end
...
function set_wan_ppoe(arg_list_table, connect)
	...
	local connectmode = tonumber(_uci_real:get("network","wan","connectmode")) or 1
	if(pppname) then
...
function get_wan_port()
	...
	local wan_ifname_arry = luci.util.split(wan_ifname, ".")
	local wan_dev = wan_ifname_arry[1]
	local wan_vlanid = wan_ifname_arry[2]
	local wan_port_id

	_uci_real:foreach("network", "switch_vlan",
	function(s)
		if(s["vlan"] == wan_vlanid) then
			wan_port_id = s["ports"]:match("^%d+")
		end
	end)
	return wan_dev, wan_port_id
end
...
function set_wan(params)
	...
	do
		...
		local wan_dev, wan_port = get_wan_port()
		if speed ~= speed_cur then
			if wan_port then
				luci.util.exec("echo phyad "..wan_port.." > /sys/kernel/debug/gmac_debug")
			end

			if speed == "auto" then
				luci.util.exec("ethtool -s "..wan_dev.." autoneg on")
			elseif speed == "10mfull" then
				luci.util.exec("ethtool -s "..wan_dev.." speed 10 duplex full")
			elseif speed == "10mhalf" then
				luci.util.exec("ethtool -s "..wan_dev.." speed 10 duplex half")
			elseif speed == "100mfull" then
				luci.util.exec("ethtool -s "..wan_dev.." speed 100 duplex full")
			elseif speed == "100mhalf" then
				luci.util.exec("ethtool -s "..wan_dev.." speed 100 duplex half")
			elseif speed == "1000mfull" then
				luci.util.exec("ethtool -s "..wan_dev.." speed 1000 duplex full")
			end

			if speed == "auto" or speed == "10mfull" or speed == "10mhalf" or speed == "100mfull" or speed == "100mhalf" or speed == "1000mfull" then
		...
		if linkstatus == "1" then
			...
			elseif wantype == 4 and connect == 0 then
				--noly in connect mode 2 will make wan down manully.
...
function get_mac()
	...
	local wanmac = string.lower(string.sub(tostring(device:mac()),1,15))..string.format("%02x",(tonumber("0x"..string.sub(device:mac(),16,17)) + 1))
...
function get_dhcp()
	...
	local dhcp_end_value = dhcp_start_value + limit - 1
...
--根据mac在devlist，wldevlist，bindtable中寻找非匿名的hostname。foreach中不能return，因此写到循环外，以节约调用时间。
function get_hostname_by_mac(mac)
	local  real_hostname

	_uci_real:foreach("bindtable", "device",
	function(s)
		if string.lower(tostring(s.mac)):gsub("_", ":") == mac then
			if(s.name and s.name ~= "undefined") then real_hostname = s.name end
		end
	end)
	if real_hostname then return real_hostname end

	_uci_real:foreach("devlist", "device",
	function(s)
		if string.lower(tostring(s.mac)):gsub("_", ":") == mac then
			if(s.hostname and s.hostname ~= "undefined") then real_hostname = s.hostname end
		end
	end)
	if real_hostname then return real_hostname end
...
--获取某ip最后一次使用距离当前的时间，以table返回。若table中不含此ip，则会得到-1
function get_ip_connect_time()
	local result = {}
	local file = io.popen("ip -f inet neigh show")
	local lines = file:read("*line")
	while lines do
		ip = lines:match("%d+%.%d+%.%d+%.%d+")
		t1,t2,t3 = lines:match("%s+(%d+)/(%d+)/(%d+)%s+")
		if ip and t1 and t2 and t3 then
			result[ip] = tonumber(t1)
		end
...
function get_ip_mac_online_table()
	...
	local result = {}
	local e, r, v
	local ip_time_table = get_ip_connect_time()
	...
	if fs.access("/proc/net/arp") then
		for e in io.lines("/proc/net/arp") do
			...
			if r[1] ~= "IP" then
				local tmp_mac = r[4]
				if(rv[tmp_mac]) then
					--同一个mac多个ip的情况：比较当前ip使用时间(rv[tmp_mac])与新获取的ip(r[1])的使用时间，获取更近一次使用的ip
					if( ip_time_table[r[1]] > 0 and ip_time_table[r[1]] < ip_time_table[rv[tmp_mac]]) then
						rv[tmp_mac] = r[1]
					end
				else
					rv[tmp_mac] = r[1]
	...
	for k,v in pairs(rv) do
		result[#result+1] = {}
		result[#result]["mac"] = string.upper(k)
		result[#result]["name"] = get_hostname_by_mac(k)
		result[#result]["ipaddr"] = v
		result[#result]["band"] = BindJudge(k, v, 1)
	end
...
function get_ip_mac_bind_table()
	...
	while true do
		...
		rv[#rv+1] = {}
		rv[#rv]["name"] = get_hostname_by_mac(string.sub(line, 1, 17))
...
function set_ip_mac_bind_table(params)
	...
	if (action == 0) then
		for k, v in pairs(arg_list_table["devices"]) do
			local name = v.name
			local line = string.lower(tostring(v.mac)):gsub("_", ":").." "..v.ipaddr
			if not check_lan_area(v.ipaddr) and not check_wan_area(v.ipaddr) and not check_guest_area(v.ipaddr) then
				code = sferr.ERROR_INPUT_PARAM_ERROR
				break
			end
			if BindJudge(string.lower(tostring(v.mac)):gsub("_", ":"), "", 0) then
				luci.util.exec("sed -i 's/%s.*/%s/g' /etc/ethers"%{string.lower(tostring(v.mac)):gsub("_", ":"), line})
				if name then
	...
	elseif (action == 1) then
		...
		io.input(file)
		for k, v in pairs(arg_list_table["devices"]) do
			luci.util.exec("sed -i '/%s/d' /etc/ethers"%{string.lower(tostring(v.mac)):gsub("_", ":")})
	...
	sysutil.fork_exec("sleep 1; env -i /bin/ubus call network reload; /etc/init.d/dnsmasq restart >/dev/null 2>/dev/null")
...
function get_ipv6()
	local wan6_proto = _uci_real:get("network","wan6","proto")
	local result = {
		code = 0,
		msg = "OK",
		lan = {},
	...
	local dhcpv6 = _uci_real:get("dhcp","lan","dhcpv6")
	local ra = _uci_real:get("dhcp","lan","ra")
	local wan6disabled = _uci_real:get("network","wan6","disabled")

	result.enable = ((dhcpv6 and dhcpv6 ~= "disabled")
			or (ra and ra ~= "disabled")
			or (wan6disabled and wan6disabled ~= "1")) and true or false

	if result.enable then
		if (wan6_proto == "dhcpv6") then
			result.wan.address_type = "auto"
		else
			result.wan.address_type = "manual"
			result.wan.ip6gw = _uci_real:get("network","wan6","ip6gw")
			ip6addr = _uci_real:get_list("network","wan6","ip6addr")
			if ip6addr and #ip6addr ~= 0 then
				result.wan.ip6addr = ip6addr[1]
			end
		end

		local dns = _uci_real:get_list("network","wan6","dns")

		if dns and (#dns ~= 0) then
			result.wan.dns_mode = "manual"
			result.wan.dns1 = dns[1]
			result.wan.dns2 = dns[2]
		else
			result.wan.dns_mode = "auto"
		end

		result.lan.router_broadcast = _uci_real:get("dhcp","lan","ra_default") and true or false

		if not (dhcpv6 or ra) then
			result.lan.dhcp_mode = "disabled"
		elseif (dhcpv6 == "disabled") and (ra == "disabled") then
			result.lan.dhcp_mode = "disabled"
		else
			local lan_ip6prefix = _uci_real:get("network","wan6","ip6prefix")
			if lan_ip6prefix and lan_ip6prefix ~= "" then
				result.lan.dhcp_mode = "manual"
				pos1 = string.find(lan_ip6prefix[1], "/", 1)
				pos2 = string.find(lan_ip6prefix[1], ",", pos1+1)
				pos3 = string.find(lan_ip6prefix[1], ",", pos2+1)
				result.lan.ip6prefix = string.sub(lan_ip6prefix[1], 1, pos1-1)
				result.lan.prefix_len = string.sub(lan_ip6prefix[1], pos1+1, pos2-1)
				result.lan.preferred_lifetime = string.sub(lan_ip6prefix[1], pos2+1, pos3-1)
				result.lan.valid_lifetime = string.sub(lan_ip6prefix[1], pos3+1)
				result.lan.dns1 = _uci_real:get_list("dhcp","lan","dns")
				result.lan.dns2 = _uci_real:get_list("dhcp","lan","domain")
			else
				result.lan.dhcp_mode = "auto"
...
function set_ipv6()
	...
	if enable == 1 then
		if arg_list_table["wan"].address_type == "auto" then
			_uci_real:set("network","wan6","proto", "dhcpv6")
			_uci_real:delete("network","wan6","ip6addr")
			_uci_real:delete("network","wan6","ip6gw")
		else
			_uci_real:set("network","wan6","proto", "static")
			_uci_real:set("network","wan6","ip6gw", arg_list_table["wan"].ip6gw)
			_uci_real:set("network","wan6","ip6addr", arg_list_table["wan"].ip6addr)
		end

		if arg_list_table["wan"].dns_mode == "auto" then
			_uci_real:set("network","wan6","dns", "")
		else
			local dns_list = {}
			local dns1 = arg_list_table["wan"].dns1
			local dns2 = arg_list_table["wan"].dns2
			if dns1 then
				dns_list[1] = dns1
			end
			if dns2 then
			...
			_uci_real:set_list("network","wan6","dns",dns_list)
		end

		if arg_list_table["lan"].router_broadcast then
			_uci_real:set("dhcp","lan","ra_default", "1")
		else
			_uci_real:set("dhcp","lan","ra_default", "")
		end

		if arg_list_table["lan"].dhcp_mode == "auto" then
			_uci_real:set("dhcp","lan","dhcpv6", "server")
			_uci_real:set("dhcp","lan","ra", "server")
			_uci_real:delete("network","wan6","ip6prefix")
			_uci_real:delete("dhcp","lan","dns")
			_uci_real:delete("dhcp","lan","domain")
		elseif arg_list_table["lan"].dhcp_mode == "disable" then
			_uci_real:set("dhcp","lan","dhcpv6", "disabled")
			_uci_real:set("dhcp","lan","ra", "disabled")
			_uci_real:delete("network","wan6","ip6prefix")
			_uci_real:delete("dhcp","lan","dns")
			_uci_real:delete("dhcp","lan","domain")
		else
			_uci_real:set("dhcp","lan","dhcpv6", "server")
			_uci_real:set("dhcp","lan","ra", "server")
			set_string = arg_list_table["lan"].ip6prefix.."/"..arg_list_table["lan"].prefix_len..","..arg_list_table["lan"].preferred_lifetime..","..arg_list_table["lan"].valid_lifetime
			_uci_real:set_list("network","wan6","ip6prefix", set_string)
			_uci_real:set_list("dhcp","lan","dns", arg_list_table["lan"].dns1)
			_uci_real:set_list("dhcp","lan","domain", arg_list_table["lan"].dns2)
		end
	else
		_uci_real:set("network","wan6", "disabled", "1")
		_uci_real:set("dhcp","lan","dhcpv6", "disabled")
		_uci_real:set("dhcp","lan","ra", "disabled")
	end
	...
	_uci_real:commit("dhcp")
	_uci_real:commit("network")
	sysutil.fork_exec("/etc/init.d/dnsmasq restart; sleep 1; env -i /bin/ubus call network reload >/dev/null 2>/dev/null")
...
function isgmac()
	local t = io.popen("lsmod | grep -c gmac")
	local num = t:read("*n")
	t:close()
	return num > 0
end

function wanhas1000mhalf()
	local t = io.popen("lsmod | grep -c eswitch")
	local num = t:read("*n")
	t:close()
	return num > 0
end
