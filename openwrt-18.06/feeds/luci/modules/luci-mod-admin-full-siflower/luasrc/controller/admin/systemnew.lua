...
local wds_enable = false;
    ...
    entry({"admin", "systemnew", "syslog"}, template("new_siwifi/device_manager/syslog") , _("system log"), 8);
  	--entry({"admin", "systemnew", "device_limit"}, template("new_siwifi/device_manager/device_limit") , _("device limit"), 9);
	...
	entry({"admin", "systemnew", "get_limit_mode_config"}, call("get_limit_mode_config")).leaf = true;
	entry({"admin", "systemnew", "set_limit_mode_config"}, call("set_limit_mode_config")).leaf = true;
	entry({"admin", "systemnew", "get_limit_mode_enable"}, call("get_limit_mode_enable")).leaf = true;
	entry({"admin", "systemnew", "set_limit_mode_enable"}, call("set_limit_mode_enable")).leaf = true;
	entry({"admin", "systemnew", "get_limit_mode_device"}, call("get_limit_mode_device")).leaf = true;
	entry({"admin", "systemnew", "remove_limit_mode_device"}, call("remove_limit_mode_device")).leaf = true;
end
...
function get_date()
	...
	local guest_limittime_en = 0
	local guest_limittime_24g = _uci_real:get("wireless", "guest_radio0", "limittime")
	local guest_limittime_5g = _uci_real:get("wireless", "guest_radio11", "limittime")

	if(guest_limittime_24g == "true" or guest_limittime_5g == "true") then
		guest_limittime_en = 1
	end
...
function get_ota_update_status()
	...
	if sysutil.sane("/tmp/upgrade_status") then
		...
		nixio.syslog("crit","upgrade_status"..tostring(info.status))
...
function get_limit_mode_enable()
	local enable = deviceImpl.get_limit_mode_enable()
	local result = {
		code = 0,
		msg = "OK",
		enable = enable,
...
function set_limit_mode_enable()
	...
	local code = deviceImpl.set_limit_mode_enable(enable)
	local result = {
		code = code,
		msg = "OK",
	}
...
function set_limit_mode_config()
	...
	deviceImpl.set_limit_mode_config(start_time, end_time, limit)
	local result = {
		code = 0,
		msg = "OK",
	}
...
function get_limit_mode_config()
	...
	local limit_info = deviceImpl.get_limit_mode_config()
	local result = {
		code = 0,
		msg = "OK",
...
function get_limit_mode_device()
	local result = {
		code = 0,
		msg = "OK",
		device = deviceImpl.get_limit_mode_device()
	}
	sysutil.sflog("INFO","data configurre changed!")
	luci.http.prepare_content("application/json")
	luci.http.write_json(result)
end

function remove_limit_mode_device(device)
	local arg_list, data_len = luci.http.content()
	local arg_list_table = json.decode(arg_list)
	local code = 0
	if arg_list_table["device"] ~= nil then
		code = devieceImpl.remove_limit_mode_device(arg_list_table["device"])
	else
		code = 1
	end
	local result = {
