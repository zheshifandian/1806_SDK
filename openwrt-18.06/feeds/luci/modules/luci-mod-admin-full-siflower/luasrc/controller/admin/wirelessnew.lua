...
-- Set txpower_lvl (now can't change it on website)
-- Default: level 2
local SAVE_MODE_TXPOWER = 0
local NORMAL_MODE_TXPOWER = 1
local PERFORMANCE_MODE_TXPOWER = 2

-- Set txpower (dBm, now use)
-- Default PERFORMANCE_MODE: 20 dBm for 2.4G, 25 dBm for 5G
local SAVE_MODE_TXPOWER_24G = 5
local NORMAL_MODE_TXPOWER_24G = 10
local PERFORMANCE_MODE_TXPOWER_24G = 20
local SAVE_MODE_TXPOWER_5G = 5
local NORMAL_MODE_TXPOWER_5G = 15
local PERFORMANCE_MODE_TXPOWER_5G = 25
...
function index()
	...
	entry({"admin", "wirelessnew", "get_wds_ifconnect"}, call("get_wds_ifconnect")).leaf = true;
...
function get_wifi_iface()
	...
	for i, dev in pairs(wifis) do
		...
		wifi_status = wifinet:get("disabled") or 0
		if tonumber(wifi_status) == 0 then
			ifaces[#ifaces]["enable"] = true
		else
			ifaces[#ifaces]["enable"] = false
		end
		ifaces[#ifaces]["ssid"]       = wifinet:get("ssid")

		if(ifaces[#ifaces]["band"] == "2.4G") then
			if tonumber(wifidev:get("txpower")) == SAVE_MODE_TXPOWER_24G then
				ifaces[#ifaces]["signal"] = SAVE_MODE
			elseif tonumber(wifidev:get("txpower")) == NORMAL_MODE_TXPOWER_24G then
				ifaces[#ifaces]["signal"] = NORMAL_MODE
			else
				ifaces[#ifaces]["signal"] = PERFORMANCE_MODE
			end
		else
			if tonumber(wifidev:get("txpower")) == SAVE_MODE_TXPOWER_5G then
				ifaces[#ifaces]["signal"] = SAVE_MODE
			elseif tonumber(wifidev:get("txpower")) == NORMAL_MODE_TXPOWER_5G then
				ifaces[#ifaces]["signal"] = NORMAL_MODE
			else
				ifaces[#ifaces]["signal"] = PERFORMANCE_MODE
			end
		...
		ifaces[#ifaces]["encryption"] = wifinet:get("encryption")
		if( wifinet:get("encryption") == "open" or  wifinet:get("encryption") == "none" ) then
...
function set_wifi_iface_local(ifaces)
	...
	if not wifinet_24g then
		wifinet_24g = wdev_2:get_wifinet("default_radio0")
	end
	for i=1,#ifaces do
			...
			if wifidev:get("band") == ifaces[i]["band"]  then
				...
				if not (ifaces[i]["band"]  == "2.4G" and freq_inter == 1) then
					...
					if ifaces[i]["open"] == true then
						...
						wifinet:set("ieee80211w",nil)
					else
						wifinet:set("encryption", ifaces[i]["encryption"])
						wifinet:set("key", ifaces[i]["key"])
						if(ifaces[i]["encryption"] == "sae") then
							wifinet:set("ieee80211w", 2)
						elseif(ifaces[i]["encryption"] == "sae-mixed") then
				...
				elseif ifaces[i]["hwmode"] == "11g" and wifidev:get("band") == "2.4G" then
					...
					elseif ( ifaces[i]["htmode"] == "HT40") then
						wifidev:set("htmode", "HT40")
						wifidev:set("ht_coex", "0")
					elseif (ifaces[i]["htmode"] == "HT20") then
				...
				if ifaces[i]["band"] == "2.4G" then
					if ifaces[i]["signal"] == SAVE_MODE then
						wifidev:set("txpower", SAVE_MODE_TXPOWER_24G)
						wifidev:set("txpower_lvl", SAVE_MODE_TXPOWER)
					elseif ifaces[i]["signal"] == NORMAL_MODE then
						wifidev:set("txpower", NORMAL_MODE_TXPOWER_24G)
						wifidev:set("txpower_lvl", NORMAL_MODE_TXPOWER)
					else
						wifidev:set("txpower", PERFORMANCE_MODE_TXPOWER_24G)
						wifidev:set("txpower_lvl", PERFORMANCE_MODE_TXPOWER)
					end
				else
					if ifaces[i]["signal"] == SAVE_MODE then
						wifidev:set("txpower", SAVE_MODE_TXPOWER_5G)
						wifidev:set("txpower_lvl", SAVE_MODE_TXPOWER)
					elseif ifaces[i]["signal"] == NORMAL_MODE then
						wifidev:set("txpower", NORMAL_MODE_TXPOWER_5G)
						wifidev:set("txpower_lvl", NORMAL_MODE_TXPOWER)
					else
						wifidev:set("txpower", PERFORMANCE_MODE_TXPOWER_5G)
						wifidev:set("txpower_lvl", PERFORMANCE_MODE_TXPOWER)
					end
				end
...
function get_wds_ifconnect()
	local arg_list_table = get_arg_list()
	local result = {}
	local code = 0
	code,result = wirelessImpl.get_wds_ifconnect(arg_list_table)
	nixio.syslog("crit", myprint(result))
	sysutil.set_easy_return(code, result)
end
...
function set_freq_intergration_impl(arg_list_table)
	...
	if not wifinet_5 then
		wifinet_5 = wdev_5:get_wifinet("default_radio1")
	end

	if not wifinet_2 then
		wifinet_2 = wdev_2:get_wifinet("default_radio0")
	end 
...
function get_freq_intergration_impl()
	...
	if not wifinet then
		wifinet = wdev_5:get_wifinet("default_radio1")
	end
	if( wifinet and tonumber(wifinet:get("cond_hidden")) == 1) then
		return 1
	else
		return 0
	end
