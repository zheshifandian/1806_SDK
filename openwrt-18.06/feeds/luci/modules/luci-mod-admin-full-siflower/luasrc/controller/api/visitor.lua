...
function get_bindinfo()
    ...
    local ret = io.popen("cat /sys/devices/platform/factory-read/product_key")
    if ret then
        routerkey = ret:read("*a")
        ret:close()
    end
    ret = io.popen("cat /sys/devices/platform/factory-read/product_key_flag")
    if ret then
    ...
    elseif(protocol <=  sysutil.get_version_local()) then
		...
		result["mac"] = uci:get("network", "lan_dev", "macaddr") or ''
