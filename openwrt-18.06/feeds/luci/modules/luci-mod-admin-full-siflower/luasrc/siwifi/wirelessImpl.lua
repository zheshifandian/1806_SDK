...
function get_ssids()
	...
	for i, dev in pairs(wifis) do
		for n=1,#dev.networks do
			...
			channels[#channels+1] = dev.networks[n].channel
		end
	end
	return ssids,channels
end
...
function get_customer_wifi_iface()
	...
	function(s)
		if string.find(s.ifname, "guest") then
			...
			local encryption = _uci_real:get("wireless", s[".name"], "encryption")
			if( encryption == "open" or encryption == "none") then
...
function set_customer_wifi_iface(arg_list_table)
	...
	if((changes ~= nil) and (type(changes) == "table") and (next(changes) ~= nil)) then
		...
		sysutil.fork_exec("sleep 1; env -i; ubus call network reload ; wifi reload_legacy; sleep 3; gwifi start; gwifi speed_rst 0;gwifi speed_rst 1; sleep 1;/etc/init.d/cron restart")
	end
...
--wifi start to connect after commit("wireless"), so move clear_wds_passwderr from wds_enable to here
	...
	--modify channel & store origin channel setting
	local oc = _uci_real:get("wireless", wifi_if.device, "channel")
	_uci_real:set("wireless", wifi_if.device, "origin_channel", oc)
	_uci_real:set("wireless", wifi_if.device, "channel", arg_list_table.channel)

	--change htmode
	if wifi_if.device == "radio0" then
		if arg_list_table.secondary_chan == 1 then
			_uci_real:set("wireless", "radio0", "htmode", "HT40+")
		elseif arg_list_table.secondary_chan == 3 then
			_uci_real:set("wireless", "radio0", "htmode", "HT40-")
		end
	end
...
function wds_getrelip(arg_list_table)
	...
	if gw_t[1] and gw_t[1] ~= "nil" and gw_t[2] and gw_t[3] then
...
-- 由于commit("wireless")会使wifi重启，有时甚至会将wds_enable接口中断，为避免这种偶然情况发生，先开始的15s检测放到了独立的接口中（get_wds_ifconnect）
-- 第二个15秒再重启一下wifi继续检测
	for retry = 2, 1, -1 do
		local passwd_err = 0
		retry_times = 1;
...
--check args
	if (arg_list_table.ifaces == nil) then
		br_ip = "nil"
	end
...
--wireless operation
		...
		-- disable interface excpet sfi0/1
		-- make sure no station connect to wlan0/wlan1 during WDS setting
		_uci_real:foreach("wireless","wifi-iface",
		function(s)
			if s.ifname ~= "sfi0" and s.ifname ~= "sfi1" then
				wifi_status = s.disabled or 0
				_uci_real:set("wireless", s[".name"], "tmp_status", wifi_status)
				_uci_real:set("wireless", s[".name"], "disabled", "1")
			end
		end)
...
--network operation
		...
		if gw_t[1] and gw_t[1] ~= "nil" and gw_t[2] and gw_t[3] then
			...
			if gateway == lanip then
				...
				lanip = gw_t[1].."."..gw_t[2].."."..seg..".".."1"
				redirect_ip = lanip
				_uci_real:set("network", "lan", "ipaddr", redirect_ip)
			end
		end
...
--other operation
		...
		_uci_real:set("firewall","@defaults[0]","flow_offloading",0)
		_uci_real:set("firewall","@defaults[0]","flow_offloading_hw",0)
		...
		--	sysutil.fork_exec("alter_lan %s delay" %{redirect_ip})
			os.execute("echo "..redirect_ip.." siwifi.cn > /etc/hosts")
		end

		-- sleep 3, during this 3s time
		-- 1: make sure wds_enable successfully return result to web
		-- 2: web can post wds_getrelip to get redirect ip before next network restart
		sysutil.fork_exec("sleep 3; lua /sbin/wds_end_setting.lua")
		code = 0
	else
		code = 1716		--wds 连接超时
		if passwd_err == 1 then
		...
		--reset channel
		local oc = _uci_real:get("wireless", "radio0", "origin_channel")
		local dev = "radio0"
		if oc == nil then
			oc = _uci_real:get("wireless", "radio1", "origin_channel")
			dev = "radio1"
		end
		_uci_real:set("wireless", dev, "channel", oc)
		_uci_real:delete("wireless", dev, "origin_channel")

		--reset htmode
		if dev == "radio0" then
			local hm = _uci_real:get("wireless", "radio0", "htmode")
			hm = string.gsub(hm, "+", "")
			hm = string.gsub(hm, "-", "")
			_uci_real:set("wireless", "radio0", "htmode", hm);
		end
...
function wds_disable(arg_list_table)
	...
	local dev = _uci_real:get("network", "wwan", "ifname")
	...
	if dev == "sfi0" then
		...
		hm = string.gsub(hm, "-", "")
		_uci_real:set("wireless", "radio0", "htmode", hm);
	elseif dev == "sfi1" then
		dev = "radio1"
	end
	oc = _uci_real:get("wireless", dev, "origin_channel")
	_uci_real:set("wireless", dev, "channel", oc)
	_uci_real:delete("wireless", dev, "origin_channel")
	...
	if arg_list_table.redirect == 1 then
		...
		sysutil.fork_exec("sleep 1;uci set network.lan.netclash=0;uci commit network;/etc/init.d/network restart")
	else
		luci.sys.call("uci set network.lan.netclash=0;uci commit network;/etc/init.d/network reload")
	end
	return code, result
end
...
function get_wds_info()
	...
	function(s)
		if(s["ifname"] == "wlan0") then
			...
			if (not info[1].ssid) or (info[1].ssid == "") then
				info[1].ssid = tostring(s["ssid"])
			end
		end
		if(s["ifname"] == "wlan1") then
			...
			if (not info[2].ssid) or (info[2].ssid == "") then
				info[2].ssid = tostring(s["ssid"])
			end
		end
...
function get_wds_ifconnect(arg_list_table)
	local result = {
		en = 0
	}
	local code = 0
...
function setwifi(arg_list_table)
	...
	if param_disableall and param_disableall == 0 or param_disableall == 1 then
		ssids,channles = get_ssids()
		setting_param_json = {}
		for i=1,#ssids do
			setting_param_json[i] = {}
			setting_param_json[i].oldssid = ssids[i]
			setting_param_json[i].channel = channels[i]
...
function getwifi_advanced()
	...
	for i, dev in pairs(wifis) do
		...
		rv[#rv]["enable"]     = wifinet:get("disabled")==nil and 1 or 0
