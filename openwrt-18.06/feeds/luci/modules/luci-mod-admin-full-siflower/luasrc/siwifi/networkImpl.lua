...
function get_wire_assocdev(deal_new, ...)
	...
	local arpscan_res
	local arp_check_dev = ... or 0
	local arp_check_dev_en = 1
	if(tonumber(arp_check_dev) == 1) then
		arp_check_dev_en = _uci_real:get("basic_setting","arp_check_dev","enable") or 0
	end
	if(tonumber(arp_check_dev_en) == 1) then
		arpscan_res = io.popen("arp-scan -I %s -f /tmp/arpscan-ip -r 3 -q -s %s -x" % {wire_ifname,lan_ip})
	end
...
function do_arp_check_device(ip, mac, br, ...)
	...
	if(tonumber(arp_check_dev_en) == 1) then
		scan_value = os.execute("arping -I %s -c 5 %s 2>&1 1>/dev/null" %{br,ip})
	end
	...
	local function do_set(mac, ip, ifname, online, port)
		local data = {}
		local lan = _uci_real:get("sidefault", "acl", "lan") or "1"
		local internet = _uci_real:get("sidefault", "acl", "internet") or "1"
...
function device_list_backstage(...)
	...
	local arp_check_dev = ... or 0
	local code = 0
	local wiredev = get_wire_assocdev("1", arp_check_dev)
	if wiredev then
		set_devlist(wiredev)
	end
...
function arp_check_dev(arg_list_table)
	...
	local arp_check_dev = arg_list_table["arp_check_dev"] or 0
	local code = 0
	local ret = 0
	local result = { }
	local is_lan = 0
	...
	if ip and mac and is_lan == 1 then
		ret = do_arp_check_device(ip, mac, br, tonumber(arp_check_dev))
		if ret == 1 then
			result["notify"] = 1
		end
	end
		...
		if os.time() > upgrade_shortest_time then
			...
			if ( not ota_info ) then
				...
				if not string.find(romversion,otaversion) then
                    local downloaderType = sysutil.getDownloadType()
                    local downloadId = sysutil.getDownloadId()
					local info = {}
					info["size"] = remote_info["size"]
					info["url"] = remote_info["path"]..downloaderType..downloadId
...
function qos_info()
	...
	result["enable"] = _uci_real:get("basic_setting","qos","enable") == "1" and 1 or 0
	return code, result
end
...
function setdevicerestrict(arg_list_table)
	...
	if string.find(dev, "lease") then
		...
		local ip = networknew.mac_to_ip((mac:gsub("_",":")):upper())
		if ip then
