...
function setpasswd(arg_list_table)
	...
	if authen_vaild == false then
		...
		code = luci.sys.user.setpasswd(username, pwd)
	end
...
function get_limit_mode_enable()
	local enable = tonumber(_uci_real:get("basic_setting","limit_mode","enable"))
	if enable == 0 then
		return 0
	else
		return 1
...
function set_limit_mode_enable(enable)
	if tonumber(enable) == 0 then
		_uci_real:set("basic_setting","limit_mode","enable",0)
	else
		_uci_real:set("basic_setting","limit_mode","enable",1)
	end
	_uci_real:save("basic_setting")
	_uci_real:commit("basic_setting")
	return 0
...
function set_limit_mode_config(start_time, end_time, limit)
	local mac_list = {}
