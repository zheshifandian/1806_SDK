...
function getHardware()
	...
	model = model and string.gsub(model, "%c", "") or "none"


	local a,t
	t = io.popen("cat /sys/devices/platform/factory-read/hw_ver_flag | awk -F \"\" ' {for(i=1; i<=NF; i++){ if (($i <= \"z\") && ( $i >= \"!\" ) && (length($i) == 1)) a=a$i }} END{print a}'")
	if not t  then
	...
	if t  then
		...
		if string.find(a,"hv") then
			t = io.popen("cat /sys/devices/platform/factory-read/hw_ver | awk -F \"\" '{for(i=1; i<=NF; i++){ if (($i <= \"z\") && ( $i >= \"!\" ) && (length($i) == 1)) a=a$i }} END {print a}'");
			if not t  then
...
function get_romtype_factory()
    ...
    local romTypeFlag = {}
    local romType = {}
    local res = io.popen("cat /sys/devices/platform/factory-read/rom_type_flag")
    if res then
        romTypeFlag = res:read("*a")
        res:close()
    end
    if string.find(romTypeFlag, "rt") then
        res = io.popen("cat /sys/devices/platform/factory-read/rom_type")
        if res then
            romType = res:read("*a")
            res:close()
        end
...
function getDownloadType()
    local download = "&downloaderType="
    local Type = 1
    local downloaderType = download..Type
    return downloaderType
end

function getDownloadId()
    local download = "&sn="
    local id = uci:get("siwifi", "hardware", "sn")
	if not id then
		return download
	end
    local downloaderId = download..tostring(id)
    return downloaderId
end
...
function getChiptype()
	...
	model = string.gsub(model, "%c", "")
	if (model == "siflower/sf16a18-fullmask") or (model == "siflower/sf19a28-fullmask") then
		return 0
	else
		return 1
	end
...
function getImgtype()
	...
	local version = fs.readfile("/etc/openwrt_release")
