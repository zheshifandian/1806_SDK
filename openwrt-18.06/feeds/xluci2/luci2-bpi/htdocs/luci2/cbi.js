(function() {
	...
	validation.types = {
		...
		'wpakey': function()
		{
			...
			for(var i = 0; i < val.length; i ++)
			{
				...
				if(ts.substring(0, 2) == "%u") {
					validation.i18n('Must void Chinese characters');
					return false;
				}
			}
			...
			validation.i18n('The password should be 8 to 63 characters or 64 hexadecimal digits.');
			return false;
		},
		...
		'range': function(min, max)
		{
			...
			// if min == 0; %d can't show '0' ?
			if(min == 0)
				validation.i18n('Must be a number between 0%d and %d');
			else
		...
		'ssid': function()
		{
			...
			for(var i = 0; i < val.length; i ++)
			{
				...
				if (char_code >= 0x0000 && char_code <= 0x007F)
					length += 1;
				else if (char_code >= 0x0080 && char_code <= 0x07FF)
					length += 2;
				else if (char_code >= 0x0800 && char_code <= 0xFFFF)
					length += 3;
				else
					length += 4;
			}
			...
			validation.i18n('Must be at most 32 characters or 10 Chinese characters');
			return false;
		},
	...
	cbi_class.AbstractValue = L.ui.AbstractWidget.extend({
		...
		render: function(sid, condensed)
		{
			...
			if (!condensed)
			{
				...
				$('<div />')
					.attr('style','width:180px; margin-top:7px; text-align:left; margin-left:0px;')
//					.addClass('col-lg-5')
	...
	cbi_class.SwitchValue = cbi_class.AbstractValue.extend({
		widget: function(sid)
		{
			var o = this.options;
			...
			var j = $('<div />');
			if (i.prop('checked')) {
				j.prop('class', 'switch_on');
			} else {
				j.prop('class', 'switch_off');
			}
			j.click(function () {
				if (i.prop('checked')) {
					j.prop('class', 'switch_on');
			...
			return j.append(this.attachEvents(sid, i));
	...
	cbi_class.CheckboxValue = cbi_class.AbstractValue.extend({
		widget: function(sid)
		{
			...
			var j = $('<div />');
			if (i.prop('checked')) {
				j.prop('class', 'checkbox_press');
			} else {
				j.prop('class', 'checkbox');
			}
			j.click(function () {
				if (i.prop('checked')) {
					j.prop('class', 'checkbox_press');
	...
	cbi_class.TypedSection = cbi_class.AbstractSection.extend({
		...
		{
			if(this.label('caption'))
				return $('<h3 />')
					.addClass('title_line')
					.attr('style','border-bottom:solid 1px #c9c9c9;')
	...
	cbi_class.Map = L.ui.AbstractWidget.extend({
		...
		renderFooter: function()
		{
			...
			return $('<div />')
				...
				.append($('<div />')
					.append(L.ui.button(L.tr('Save & Apply'))
						.removeClass('btn btn-default')
						.addClass('common_button_long')
		...
		render: function()
		{
			var map0 = $('<form />').append($('<div />'));
			var map = map0.find('div');
			$('#maincontent').css('background-color', 'white');

			//if emptytop, use no title and margin-top xx px
			if(typeof(this.options.emptytop) == 'string')
				map.attr('style', 'margin-top:%s;'.format(this.options.emptytop));
			else
				map.addClass('thirdTitleContent');

			if (this.options.topline == true)
				map.attr('style','border-bottom:solid 1px #c9c9c9;');

			//if center, title located in center
			if (typeof(this.options.caption) == 'string' && this.options.center != true)
				map.append($('<h2 />')
					.text(this.options.caption));
			...
			map0.append(this.renderBody());

			if (this.options.pageaction !== false)
				map0.append(this.renderFooter());
